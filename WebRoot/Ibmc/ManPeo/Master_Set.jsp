...
<head>
    ...
    <script type="text/javascript">
        ...
        function subForm(){
        	$("#form1").attr("action",'<c:url value='/Man/Peo/savemanpeo_master.shtml'/>').trigger("submit");
        ...
        function chkIdCard(){
        	...
        	$.ajax({
				url:"<c:url value='/Man/Peo/findManPeoBeanByJq.shtml'/>",   //通过idcard查询人员信息返回bean信息记录
				...
				data:  querymap,
				error:function(data){
					parent.$.jBox.tip("与数据库连接失败！请刷新后重试！", 'warning');
				},
				success:function(data){
					if(data[0].id=="-1"){   //表示该条人员记录不存在查询不出结果集[系统在后台将会自动将其主键ID设置为-1]
						...
						if(ownersign == '${ownersign}'){   //表明该业主人员的身份已存在无法进行新增
			    			parent.$.jBox.tip("证件号码重复！", 'error');
						}else{   //表明该人员在系统中存在只不过系统人员的身份为租客
							$("#rentsign").val(rentsign);
